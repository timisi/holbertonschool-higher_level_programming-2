...
def matrix_mul(m_a, m_b):
    """matrix_mul function that multiply all elements of a matrix.
    Args:
    ...
    inner = []
    result = 0
    for times in range(len(m_a)):
        for colum_a in range(len(m_b[0])):
            for aux in range(len(m_b)):
                result += m_a[times][aux] * m_b[aux][colum_a]
            inner.append(result)
            result = 0
        out.append(inner)
        inner = []
